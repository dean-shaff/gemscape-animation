    ...
    return (
      <Gemscape svg={parsed.svg} rect={parsed.rect} g={parsed.g} ref={gemscapeRef}>
        {springs.map((props, idx) => {
          ...
          return (
            <g key={idx}>
              <AnimatedGem {...parsed.paths[idx]} ref={ref => pathRefs[idx] = ref} fillOpacity={fillOpacityInterp} {...rest} filter={filter}/>
              {/*<AnimatedGem {...parsed.paths[idx]} ref={ref => pathRefs[idx] = ref} {...rest} filter={`url(#shadow-${idx})`}/>*/}
            </g>
        )})}
      </Gemscape>
    )
